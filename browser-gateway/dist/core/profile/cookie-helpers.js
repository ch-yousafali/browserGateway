 ...
 */
export async function captureCookiesViaTransient(wsUrl, timeoutMs = 10_000) {
    ...
    try {
        return await withDeadline((async () => {
            ...
            return res?.cookies ?? [];
        })(), timeoutMs, "captureCookiesViaTransient");
    }
    finally {
 ...
 */
export async function injectCookiesViaTransient(wsUrl, cookies, timeoutMs = 10_000) {
    if (cookies.length === 0)
        return;
    const client = new WsCDPClient();
    try {
        ...
        })(), timeoutMs, "injectCookiesViaTransient");
    }
    finally {
        await client.close().catch(() => undefined);
    }
 ...
 * an oversized cookie.
 */
export function isInjectableCookie(c, nowSecs = Date.now() / 1000) {
    if (c.sameSite === "None" && !c.secure)
        return false;
    if (c.expires !== undefined && c.expires > 0 && c.expires < nowSecs)
        return false;
    if (c.partitionKeyOpaque === true)
        return false;
    if (c.name.startsWith("__Host-")) {
        if (!c.secure || c.path !== "/" || c.domain.startsWith("."))
            return false;
    }
    ...
    if (c.name.length + c.value.length > MAX_COOKIE_BYTES)
        return false;
    return true;
}
/**
 ...
 */
export function sanitizeCookiesForInject(cookies, nowSecs = Date.now() / 1000) {
 ...
 */
export function prepareCookieForInject(c) {
    const out = {
        name: c.name,
        value: c.value,
        domain: c.domain,
    ...
    if (c.sourceScheme && c.sourceScheme !== "Unset")
        out.sourceScheme = c.sourceScheme;
    if (c.sourcePort && c.sourcePort > 0)
        out.sourcePort = c.sourcePort;
    if (c.sameParty !== undefined)
        out.sameParty = c.sameParty;
