 ...
 */
export async function injectState(cdp, profile, opts = {}) {
    ...
    for (const [origin, data] of Object.entries(profile.storage)) {
        ...
        try {
            ...
            const result = (await withTimeout(cdp.send("Runtime.evaluate", {
                ...
                returnByValue: true,
                awaitPromise: false,
            }), evalTimeout, `Runtime.evaluate(write @${origin})`));
...
function buildStorageWriteExpression(data) {
