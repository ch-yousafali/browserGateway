...
export function encodeBlob(dek, dekVersion, plaintext, profileId, opts = {}) {
...
export function decodeBlobHeader(blob) {
...
export function decodeBlob(blob, dek, expectedProfileId) {
    const header = decodeBlobHeader(blob);
    const aadStr = header.aad.toString("utf8");
    if (aadStr !== expectedProfileId) {
        throw new Error(`profile id mismatch: blob AAD is "${aadStr}" but expected "${expectedProfileId}" (possible swap attack)`);
    }
