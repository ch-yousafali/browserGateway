...
export function isMarkerCookie(cookie) {
    if (cookie.name !== MARKER_NAME)
        return false;
...
export function encodeMarker(m) {
...
export function decodeMarker(value) {
    try {
        ...
        if (typeof obj !== "object" || obj === null)
            return null;
        const rec = obj;
        if (typeof rec.profileId !== "string" || typeof rec.injectedAtMs !== "number")
            return null;
        return {
...
export function filterMarkerCookies(cookies) {
...
export function isMarkerOrigin(origin) {
...
export function stripMarkerFromStorage(localStorageMap) {
...
export function stripMarkerOrigin(storage) {
    for (const key of Object.keys(storage)) {
        if (isMarkerOrigin(key))
            delete storage[key];
    }
    return storage;
}
