 ...
 */
export async function runPageAction(cdp, sessionId, options, action, runOpts = {}) {
    ...
    checkAborted(options.signal);
    if (options.viewport) {
        await cdp.sendOn("Emulation.setDeviceMetricsOverride", {
    ...
    try {
        const navResult = (await cdp.sendOn("Page.navigate", { url: options.url, transitionType: "typed" }, sessionId));
        ...
        if (waitUntil !== "commit") {
            try {
                ...
                if (!runOpts.tolerateGotoTimeout)
                    throw err;
            }
        }
    }
    ...
    checkAborted(options.signal);
    const resolvedUrl = await evaluateInPage(cdp, sessionId, "window.location.href");
    checkAborted(options.signal);
    const actionStart = Date.now();
    const data = await withTimeout(action(cdp, sessionId), DEFAULT_ACTION_TIMEOUT_MS, "action timed out", options.signal);
 ...
 */
export async function captureScreenshot(cdp, sessionId, opts = {}) {
    const params = {
    ...
    const result = (await cdp.sendOn("Page.captureScreenshot", params, sessionId));
 ...
 */
export async function capturePageHTML(cdp, sessionId) {
    return evaluateInPage(cdp, sessionId, "document.documentElement.outerHTML");
}
/**
 ...
 */
export async function evaluateInPage(cdp, sessionId, expression, opts = {}) {
    const result = (await cdp.sendOn("Runtime.evaluate", {
        expression,
 ...
 */
export async function captureElementScreenshot(cdp, sessionId, selector, opts = {}) {
    const rect = await evaluateInPage(cdp, sessionId, `(() => {
 ...
 */
export async function scrollThroughPage(cdp, sessionId) {
    await evaluateInPage(cdp, sessionId, `(async () => {
...
async function waitForSelector(cdp, sessionId, selector, timeoutMs, signal) {
    const deadline = Date.now() + timeoutMs;
    const expr = `!!document.querySelector(${JSON.stringify(selector)})`;
    while (Date.now() < deadline) {
        checkAborted(signal);
        const present = await evaluateInPage(cdp, sessionId, expr);
        if (present)
            return;
        await new Promise((resolve) => setTimeout(resolve, SELECTOR_POLL_INTERVAL_MS));
    }
...
async function waitForNavigation(cdp, sessionId, waitUntil, frameId, timeoutMs, signal) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const handler = (params) => {
            const p = params;
            if (frameId && p.frameId && p.frameId !== frameId)
                return;
            if (waitUntil === "networkidle" && p.name !== "networkIdle")
                return;
            cleanup();
            resolve();
        };
...
async function withTimeout(p, timeoutMs, label, signal) {
    let timer;
