 ...
 */
export function isEligibleForProfile(config, requestedProfile) {
    if (config.multiProfile)
        return true;
 ...
 */
export function isEligibleProviderForProfile(provider, requestedProfile) {
    if (provider.detectedKind === "browserserve")
        return true;
    if (requestedProfile == null)
        return provider.config.profile == null;
 ...
 */
export function effectiveMaxConcurrent(provider) {
...
export function hasFreeSlot(provider) {
