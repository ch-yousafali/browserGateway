...
export async function testConnectionWithClient(client, timeoutMs = 5_000) {
    const started = Date.now();
    try {
        const result = await Promise.race([
            client.sendOn("Browser.getVersion", {}, undefined),
            new Promise((_, reject) => setTimeout(() => reject(new Error(`Browser.getVersion timeout after ${timeoutMs}ms`)), timeoutMs)),
        ]);
        return {
