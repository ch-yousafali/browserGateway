...
export class SessionPool {
    ...
    constructor(gatewayPort, logger, config, token) {
    ...
    async start() {
        this.maintenanceTimer = setInterval(() => this.maintenance(), Math.min(this.config.idleTimeoutMs, 30000));
        ...
        this.logger.info({ maxSessions: this.config.maxSessions, maxPagesPerSession: this.config.maxPagesPerSession }, "pool: started");
    }
    /**
    ...
    async releasePage(handle) {
        this.activeHandles.delete(handle.id);
        await handle.context.close().catch((err) => {
            this.logger.debug({ handleId: handle.id, error: err.message }, "pool: context close failed");
        });
     ...
     * status.
     */
    async acquirePinnedPage(providerId) {
        ...
        const handle = { id: handleId, sessionId: id, context, page, acquiredAt: Date.now() };
        this.activeHandles.set(handleId, handle);
    ...
    getStatus() {
        ...
        return {
            ...
            totalActivePages: Array.from(this.sessions.values()).reduce((sum, s) => sum + s.activePages, 0),
            config: this.config,
            sessions,
        };
    }
    async shutdown() {
        ...
        const closePromises = Array.from(this.sessions.values()).map((s) => this.closeSession(s));
        await Promise.allSettled(closePromises);
        this.logger.info("pool: shut down");
    }
    // --- Proactive scaling ---
    ensureCapacity() {
        if (this.closed || this.creatingSession)
            return;
        ...
        if (totalManaged >= this.config.maxSessions)
            return;
        ...
        if (availableSlots < threshold) {
            this.logger.info({ availableSlots, threshold, activeSessions, totalManaged, maxSessions: this.config.maxSessions }, "pool: proactive scale-up");
            this.createSession().catch((err) => {
                this.logger.warn({ error: err.message }, "pool: proactive session creation failed");
            });
        }
    }
    // --- Internal ---
    async createSession() {
        ...
        try {
            ...
            session.browser = browser;
            session.state = "active";
            this.logger.info({ sessionId: id, totalSessions: this.sessions.size }, "pool: session created");
            ...
            this.sessions.delete(id);
            this.logger.error({ sessionId: id, error: err.message }, "pool: session creation failed");
            throw err;
        }
        finally {
    ...
    findAvailableSession() {
        ...
        for (const session of this.sessions.values()) {
            if (session.state !== "active")
                continue;
            if (session.activePages >= this.config.maxPagesPerSession)
                continue;
    ...
    async closeSession(session) {
        ...
        try {
            ...
            this.logger.debug({ sessionId: session.id, error: err.message }, "pool: browser close failed");
        }
        this.logger.info({
            sessionId: session.id,
            totalPagesServed: session.totalPagesServed,
    ...
    maintenance() {
        ...
        for (const session of sessionsSnapshot) {
            if (session.state === "active" &&
                ...
                this.logger.info({ sessionId: session.id, uptimeMs: now - session.createdAt }, "pool: session exceeded max lifetime, retiring");
                this.retireSession(session);
                continue;
            }
            if (session.state === "active" &&
                ...
                if (activeSessions > this.config.minSessions) {
                    this.logger.info({ sessionId: session.id, idleMs: now - session.lastActivity }, "pool: closing idle session");
                    this.retireSession(session);
                }
            }
            if (session.state === "retiring" && session.activePages === 0) {
        ...
        for (const handle of staleHandles) {
            this.logger.warn({ handleId: handle.id, sessionId: handle.sessionId, heldMs: now - handle.acquiredAt }, "pool: force-releasing page held too long");
