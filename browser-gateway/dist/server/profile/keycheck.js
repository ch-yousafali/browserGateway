...
import { KeycheckSchema, computeKcv, deriveKek, kekFingerprint, newDek, newKdfParams, unwrapDek, wrapDek, } from "../../core/profile/index.js";
export const KEYCHECK_FILE = ".keycheck";
export class KeycheckMismatchError extends Error {
    storePath;
    storedFingerprint;
    providedFingerprint;
    constructor(storePath, storedFingerprint, providedFingerprint) {
        super(`Encryption key does not match existing profiles at ${storePath}. ` +
        ...
        this.providedFingerprint = providedFingerprint;
        this.name = "KeycheckMismatchError";
    }
}
export async function readKeycheck(storePath) {
...
export async function writeKeycheck(storePath, kc) {
    const path = join(storePath, KEYCHECK_FILE);
    await writeFileAtomic(path, JSON.stringify(kc, null, 2), { mode: 0o600 });
}
export async function initStore(storePath, password) {
...
export async function openStore(storePath, password) {
    const kc = await readKeycheck(storePath);
    if (!kc) {
        throw new Error(`No .keycheck at ${storePath}. Run "browser-gateway profile init" first, or set BG_PROFILE_STORE_PATH to an initialized location.`);
    }
...
export async function rewrapKeycheck(storePath, oldPassword, newPassword) {
