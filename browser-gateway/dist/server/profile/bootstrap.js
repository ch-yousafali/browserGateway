 ...
 */
export async function bootstrapProfiles(config, logger) {
    if (!config.enabled) {
    ...
    const opened = existsSync(keycheckPath)
        ? await openStore(storePath, password).catch((err) => {
            throw new ProfileBootstrapError(err instanceof Error ? err.message : String(err), `If you intentionally changed the encryption key (env or ${resolved.path ?? "data dir"}), run "browser-gateway profile key rewrap" to migrate.\n` +
                `If you want to start over (DESTROYS PROFILES), delete ${keycheckPath}.`);
        })
        : await initStore(storePath, password).catch((err) => {
            throw new ProfileBootstrapError(`failed to initialise profile store at ${storePath}: ${err instanceof Error ? err.message : String(err)}`);
        });
    const store = new FilesystemProfileStore({ storePath });
    const lifecycle = new ProfileLifecycle(store, opened.dekByVersion, opened.currentDekVersion, logger, {
 ...
 */
export function resolveStorePath(configPath) {
