 ...
 */
export function resolveEncryptionKey(logger) {
    ...
    if (fromEnv && fromFile && fromEnv !== fromFile) {
        logger?.warn({ keyFile: filePath }, "BG_ENCRYPTION_KEY env var differs from existing key file — env wins. Profiles encrypted with the file key will not be decryptable.");
    }
    if (fromEnv) {
    ...
    logger?.info({ keyFile: filePath }, "encryption: generated new key, persisted to data directory");
