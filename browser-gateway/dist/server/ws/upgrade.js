...
import { browserserveHttp, dropOffProfile, fromBrowserservePayload, pickUpProfile, toBrowserservePayload, withProfileToken, } from "../profile/browserserve-channel.js";
...
function anyProviderEligibleForProfile(gateway, profileId) {
...
function buildPluginList(inputs) {
    const plugins = [];
    if (inputs.acquired && !inputs.isBrowserserveProfile && inputs.profileLifecycle) {
        plugins.push(makeProfilePluginFromAcquired(inputs.acquired, inputs.profileLifecycle, inputs.logger, { providerId: inputs.providerId }));
    }
...
function respondError(socket, status, body) {
...
export function createWebSocketHandler(gateway, logger, token, reconnectRegistry, profileLifecycle, transport = new NodeTcpPipeTransport(), pipelineReplay) {
    ...
    async function handleUpgrade(req, socket, head) {
        ...
        const readOnly = ["1", "true", "yes"].includes((url.searchParams.get("readOnly") ?? "").toLowerCase());
        let acquired = null;
        if (profileId !== null) {
            if (!profileLifecycle) {
            ...
            if (readOnly) {
                ...
                try {
                    ...
                    logger.info({ profileId, readOnly: true, isExisting: acquired.isExisting, cookies: acquired.cookies.length }, "profile lifecycle: acquired (read-only)");
                }
                catch (err) {
                    ...
                    logger.error({ profileId, error: err instanceof Error ? err.message : String(err) }, "profile acquire failed");
                ...
                for (;;) {
                    try {
                        ...
                        logger.info({ profileId, isExisting: acquired.isExisting, cookies: acquired.cookies.length }, "profile lifecycle: acquired");
                        break;
                    }
                    catch (err) {
        ...
        if (reconnectSessionId && reconnectRegistry) {
            ...
            if (!parked) {
                ...
                if (!provider) {
                    ...
                    const connected = await pipeToProvider(gateway, logger, transport, socket, head, req, reconnectSessionId, provider, reconnectRegistry, profileLifecycle, acquired);
                    if (connected) {
                        acquired = null;
                        return;
                    }
        ...
        const tryConnect = async () => {
            ...
            for (const provider of candidates) {
                ...
                if (plugins.length > 0) {
                    ...
                    connected = await pipeToProvider(gateway, logger, transport, socket, head, req, sessionId, provider, reconnectRegistry, profileLifecycle, acquired);
                }
                if (connected) {
                    acquired = null;
                    return true;
        ...
        try {
            ...
            if (slotAvailable && await tryConnect())
                return;
            logger.warn({ sessionId, queueSize: gateway.queueSize, targetProviderId, profileId }, "connection failed, all providers exhausted");
            if (lastResidueError) {
...
const cdpUrlCache = new Map();
const CDP_CACHE_TTL = 30000;
const CDP_CACHE_MAX = 256;
async function cachedResolveWsUrl(providerUrl, timeoutMs, headers) {
    ...
    if (resolved !== providerUrl) {
        if (cdpUrlCache.size >= CDP_CACHE_MAX) {
            ...
            if (oldestKey !== undefined)
                cdpUrlCache.delete(oldestKey);
        }
        cdpUrlCache.set(providerUrl, { wsUrl: resolved, resolvedAt: Date.now() });
    }
    return resolved;
}
async function pipeToProvider(gateway, logger, transport, clientSocket, head, req, sessionId, provider, reconnectRegistry, profileLifecycle, acquired) {
    let resolvedUrl;
    try {
        resolvedUrl = await cachedResolveWsUrl(provider.config.url, gateway.config.gateway.connectionTimeout, provider.config.headers);
    }
    catch {
    ...
    if (acquired && profileLifecycle) {
        if (isBrowserserve) {
            try {
                ...
                logger.warn({ sessionId, providerId: provider.id, error: err instanceof Error ? err.message : String(err) }, "browserserve profile drop-off failed, trying next provider");
                return false;
            }
        }
        else {
    ...
    const onTerminalClose = (reason) => {
        ...
        if (reconnectRegistry && session) {
            reconnectRegistry.park(sessionId, provider.id, provider.config.url, session.connectedAt, session.messageCount);
        ...
        if (acquired && profileLifecycle) {
            ...
            if (capturedAcquired.readOnly) {
                ...
                pickUpProfile(base, authToken, token)
                    ...
                    .catch((err) => {
                    logger.warn({ profileId: capturedAcquired.profileId, error: err instanceof Error ? err.message : String(err) }, "browserserve profile pick-up failed");
    ...
    if (!relayResult.connected) {
        ...
        if (reason?.kind === "upstream-rejected") {
            logger.warn({ sessionId, providerId: provider.id, status: reason.status, response: reason.body?.slice(0, 200) }, "provider rejected upgrade");
            ...
            logger.warn({ sessionId, providerId: provider.id, error: reason.error.message }, "provider connection failed");
        }
        return false;
    }
    return true;
}
function closeReasonToSource(reason) {
    switch (reason.kind) {
        case "client-closed":
            return "client";
        case "client-error":
