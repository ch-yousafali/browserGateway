...
export async function handlePipelineRelay(opts) {
    const { gateway, logger, req, socket, head, provider, sessionId, plugins } = opts;
    let upstreamUrl;
    try {
        upstreamUrl = await resolveWsUrl(provider.config.url, gateway.config.gateway.connectionTimeout, provider.config.headers);
    }
    catch {
        upstreamUrl = provider.config.url;
    }
    const outbound = resolveProviderOutbound(upstreamUrl, provider.config.headers);
    logger.info({ sessionId, providerId: provider.id, plugins: plugins.map((p) => p.name) }, "pipeline: connecting to provider");
    const upstreamOpen = await openUpstream(outbound.upstreamUrl, gateway.config.gateway.connectionTimeout, outbound.upstreamHeaders);
    if (!upstreamOpen.ok) {
    ...
    if (!startResult.ok) {
        ...
        logger.warn({ sessionId, providerId: provider.id, plugin: startResult.plugin }, "pipeline plugin setup failed, trying next provider");
        return { connected: false };
    }
    ...
    const result = await pipeline.run(client);
    ...
    if (opts.reconnectRegistry) {
        opts.reconnectRegistry.park(sessionId, provider.id, provider.config.url, startTime, result.counters.messageCount);
    }
    gateway.emit("session.ended", { sessionId, providerId: provider.id, durationMs });
    logger.info({ sessionId, providerId: provider.id, durationMs, reason: result.reason, ...result.counters }, "pipeline session ended");
    return { connected: true };
}
//# sourceMappingURL=pipeline-relay.js.map