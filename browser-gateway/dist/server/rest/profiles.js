...
import { decodeBlob, decodeBlobHeader, encodeBlob, PROFILE_ID_REGEX, PROFILE_VERSION, } from "../../core/profile/index.js";
import { capturedProfileToStorageState, storageStateToCapturedProfile, } from "../../core/profile/playwright-format.js";
...
const REASON_CONFIG_OFF = "Profiles are not enabled. Click 'Enable Profiles' in the dashboard, then restart the gateway.";
const REASON_BOOTSTRAP_FAILED = (err) => `Profiles are enabled in gateway.yml but the bootstrap failed at startup: ${err}. The most common cause is a stale keycheck in the data directory — delete ${"`"}$BG_DATA_DIR/profiles${"`"} and restart to reinitialize.`;
/**
 ...
 */
export function createDisabledProfileRoutes(deps = {}) {
    const app = new Hono();
    const reason = () => deps.bootstrapError ? REASON_BOOTSTRAP_FAILED(deps.bootstrapError) : REASON_CONFIG_OFF;
    app.get("/profiles", (c) => c.json({ enabled: false, count: 0, profiles: [], reason: reason(), bootstrapError: deps.bootstrapError ?? null }));
    app.get("/profiles/:id", (c) => c.json({ error: "Profile not found", reason: reason() }, 404));
    app.delete("/profiles/:id", (c) => c.json({ error: "Profiles disabled", reason: reason() }, 400));
    app.get("/profiles/:id/export", (c) => c.json({ error: "Profiles disabled", reason: reason() }, 400));
    app.post("/profiles/import", (c) => c.json({ error: "Profiles disabled", reason: reason() }, 400));
    app.post("/profiles/create", (c) => c.json({ error: "Profiles disabled", reason: reason() }, 400));
    /**
    ...
    app.post("/profiles/disable", makeToggleHandler(() => deps.config, disableProfilesFlow, "Cannot toggle profiles without a loaded config", "Disable failed"));
    return app;
}
/**
...
export function createProfileRoutes(deps) {
    ...
    app.delete("/profiles/:id", async (c) => {
        ...
        if (!token) {
            return c.json({ error: "Profile is in use by an active session" }, 409);
        }
        try {
    ...
    app.get("/profiles/:id/export", async (c) => {
        ...
        if (format === "playwright") {
            ...
            try {
                ...
                return c.json({ error: `Blob header unreadable: ${err instanceof Error ? err.message : String(err)}` }, 500);
            }
            const dek = dekByVersion.get(header.dekVersion);
            if (!dek) {
                return c.json({ error: `Blob requires DEK version ${header.dekVersion} which is not in this gateway's key ring` }, 400);
            }
            let captured;
            try {
                const plaintext = decodeBlob(blob, dek, id);
                captured = JSON.parse(plaintext.toString("utf8"));
            }
            catch (err) {
                return c.json({ error: `Failed to decrypt profile: ${err instanceof Error ? err.message : String(err)}` }, 500);
            }
    ...
    app.post("/profiles/import", async (c) => {
        ...
        if (format === "playwright") {
            ...
            try {
                ...
                return c.json({ error: `Invalid Playwright storageState: ${err instanceof Error ? err.message : String(err)}` }, 400);
            }
            const dekVersions = Array.from(dekByVersion.keys());
            if (dekVersions.length === 0) {
                return c.json({ error: "No encryption keys configured" }, 500);
            ...
            const token = await store.lock(id, 5_000);
            if (!token) {
                return c.json({ error: "Profile is in use by an active session — try again later" }, 409);
            }
            try {
                await store.putRaw(id, encoded.bytes);
                logger.info({ profileId: id, bytes: encoded.totalLen, source: "playwright" }, "profile imported via REST (playwright storageState)");
        ...
        try {
            if (contentType.includes("application/octet-stream") || contentType === "") {
                ...
                return c.json({ error: "Content-Type must be application/octet-stream (for bgp) or application/json (for playwright)" }, 415);
            }
        }
        catch {
        ...
        try {
            ...
            return c.json({ error: `Invalid profile blob: ${err instanceof Error ? err.message : String(err)}` }, 400);
        }
        ...
        try {
            ...
            return c.json({ error: `Blob failed integrity check: ${err instanceof Error ? err.message : String(err)}` }, 400);
        }
        ...
        try {
            ...
            logger.info({ profileId, bytes: blob.length, dekVersion: header.dekVersion }, "profile imported via REST");
