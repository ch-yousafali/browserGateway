...
export function createRestRoutes(pool, gateway, logger, profileLifecycle) {
    const rest = new Hono();
    /**
     ...
     */
    const providerGate = async (c, next) => {
        ...
        const now = Date.now();
        const allDown = providers.every((p) => (p.cooldownUntil && p.cooldownUntil > now) || !p.healthy);
        if (allDown) {
