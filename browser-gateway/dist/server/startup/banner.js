...
const ANSI_CSI_PATTERN = new RegExp(String.fromCharCode(27) + "\\[\\d+(;\\d+)*m", "g");
function pad(s, n) {
...
export function printStartupBanner(opts) {
    const { version, port, hasDashboard, authEnabled, profilesStatus, providers, readyMs, config, } = opts;
    const base = `http://localhost:${port}`;
    const wsBase = `ws://localhost:${port}`;
    const lines = [];
    lines.push("");
    lines.push(`  ${c.bg}▲${c.reset} ${c.bold}browser-gateway${c.reset} ${c.gray}v${version}${c.reset}  ${c.dim}ready in ${readyMs}ms${c.reset}`);
    lines.push("");
    lines.push(arrow("Gateway:", base));
    if (hasDashboard)
        lines.push(arrow("Dashboard:", `${base}/web`));
    ...
    lines.push(row("Providers:", total === 0
    ...
    if (total > 0 && healthyCount < total) {
        lines.push(`    ${c.yellow}${healthyCount}/${total} healthy${c.reset}`);
    }
    if (total > 0)
        lines.push("");
    lines.push(row("Profiles:", profilesStatus === "enabled"
        ? `${c.green}enabled${c.reset}`
        : `${c.gray}disabled${c.reset}  ${c.dim}(set profiles.enabled: true in gateway.yml)${c.reset}`));
    lines.push(row("Auth:", authEnabled
    ...
    lines.push(row("MCP:", `${c.green}enabled${c.reset}  ${c.dim}(POST ${base}/mcp)${c.reset}`));
    const pool = config.pool;
    lines.push(row("Pool:", `${c.dim}${pool.minSessions}..${pool.maxSessions} sessions, ${pool.maxPagesPerSession} pages/session${c.reset}`));
    lines.push("");
