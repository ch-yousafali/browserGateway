...
async function startServer() {
    ...
    if (resolvedPort !== undefined)
        config.gateway.port = resolvedPort;
    const logger = pino({
    ...
    try {
        ...
        profileBootstrap = { enabled: false };
    }
    ...
    const app = createApp(gateway, token, webDir, logger, pool, profileBootstrap.enabled
        ? {
            store: profileBootstrap.store,
            dekByVersion: profileBootstrap.dekByVersion,
            lifecycle: profileBootstrap.lifecycle,
        }
        : undefined, profileBootstrapError, replayStore, resolveDataDir(), reconnectRegistry);
    if (token) {
    ...
    const { handleUpgrade } = createWebSocketHandler(gateway, logger, token, reconnectRegistry, profileBootstrap.enabled ? profileBootstrap.lifecycle : undefined, undefined, { storePath: replayStorePath, replayConfig: config.replay });
    ...
    void gateway.registry.awaitInitialProbes({ maxWaitMs: 5_000 }).then(() => {
        for (const provider of gateway.registry.getAll()) {
            if (provider.config.multiProfile !== true)
                continue;
            if (provider.detectedKind === "browserserve")
                continue;
            logger.error({
    ...
    server.listen(config.gateway.port, bindHost, async () => {
        ...
        logger.info({ port: config.gateway.port, host: bindHost, providers: gateway.registry.size() }, `browser-gateway running on http://localhost:${config.gateway.port}`);
