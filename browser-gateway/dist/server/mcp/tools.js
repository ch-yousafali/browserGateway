...
export function registerTools(server, gateway, sessionManager, _logger) {
    ...
    server.registerTool("browser_navigate", {
        ...
        description: "Navigate the browser to a URL. A browser session is automatically created if none exists. " +
        ...
        if (!session)
            return err("Failed to create browser session - all providers unavailable.");
        try {
            ...
            return err(`Navigation error: ${e.message}`);
        }
    });
    server.registerTool("browser_snapshot", {
        title: "Get Page Snapshot",
        description: "Get an accessibility snapshot of the current page. Returns numbered refs like [1], [2] " +
        ...
        const session = await getOrCreateSession(sessionId);
        if (!session)
            return err("No active session. Call browser_navigate first.");
        try {
    ...
    server.registerTool("browser_screenshot", {
        ...
        description: "Capture a screenshot of the current page. Returns a PNG image. " +
    ...
    server.registerTool("browser_set_viewport", {
        ...
        description: "Set the browser viewport dimensions. Use this before taking screenshots to control the page layout " +
        ...
        try {
            ...
            return err(`Set viewport failed: ${e.message}`);
        }
    });
    server.registerTool("browser_interact", {
        title: "Interact with Page",
        description: "Perform an action on the page: click, type, select, press key, or scroll. " +
        ...
        `);
            ...
            if (action === "type") {
                if (!selector)
                    return err("'selector' is required for type action.");
        ...
        `);
            ...
            if (action === "select") {
                if (!selector)
                    return err("'selector' is required for select action.");
                if (inputText === undefined)
                    return err("'text' is required for select action (the option value).");
                await session.cdp.evaluate(`
          (() => {
            const el = document.querySelector(${JSON.stringify(selector)});
            if (!el) throw new Error('Element not found: ' + ${JSON.stringify(selector)});
        ...
        `);
            ...
            return err(`Interact failed: ${e.message}`);
        }
    });
    server.registerTool("browser_evaluate", {
        title: "Evaluate JavaScript",
        description: "Execute JavaScript code in the browser page. Use for any operation not covered by other tools. " +
    ...
    server.registerTool("browser_close", {
        ...
        if (!result.success)
            return err(`Session not found: ${targetId}`);
